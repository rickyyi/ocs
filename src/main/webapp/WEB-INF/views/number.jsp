...
<script type="text/javascript"> 
	...
	function doEdit(){
		...
		// 赋值
		$("#updateNumberForm input[name=id]").val(rows[0].id);
		$("#updateNumberForm input[name=number]").val(rows[0].number);
		$("#updateNumberForm input[name=grouptype]").val(rows[0].grouptype);
		$("#updateNumberForm input[name=importtime]").val(rows[0].importtime);
		$("#updateNumberForm input[name=updatetime]").val(rows[0].updatetime);
		// 打开窗口
		$('#updateNumberWindow').window("open");
	}
	...
	function doDelete(){
		...
		$.ajax({
			type:"post",
			url:"${pageContext.request.contextPath }/number_deleteNumber",
			data:{"ids":ids},
	...
	$(function(){
		...
		$('#grid').datagrid( {
			...
			url : "${pageContext.request.contextPath }/json/number.json",
			idField : 'id',
			columns : columns,
		...
		var myUpload = $("#button-import").upload({
	        name: 'upload',
	        action: '${pageContext.request.contextPath}/number_importData',//提交excel表格到数据库
		...
		$("#save").click(function(){
			if ($("#numberForm").form("validate")) {
				...
				$("#numberForm").form("clear");
		});
	
		// 修改号码
		$("#update").click(function(){
			if ($("#updateNumberForm").form("validate")) {
				$("#updateNumberForm").submit();
			} 
				$("#updateNumberForm").form("clear");
		});
	...
	function doDblClickRow(rowIndex, rowData){
		$("#updateNumberForm").form("load",rowData);
		$('#updateNumberWindow').window("open");
	};
...
<body class="easyui-layout" style="visibility:hidden;">
	...
	<!-- 添加号码 -->
	<div class="easyui-window" title="添加号码" id="addNumberWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		...
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="numberForm" action="${pageContext.request.contextPath }/number_savenNumber" method="post">
				<table class="table-edit" width="80%" align="center">
					<tr class="title">
						<td colspan="2">号码信息</td>
					</tr>
					<tr>
						<td>号码</td>
					...
					<tr>
						<td>组别</td>
						<td><input type="text" name="grouptype" class="easyui-validatebox" required="true"/></td>
					</tr>
				</table>
			</form>
		</div>
	...
	<!-- 修改 号码 -->
	<div class="easyui-window" title="修改号码组" id="updateNumberWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		<div style="height:31px;overflow:hidden;" split="false" border="false" >
			<div class="datagrid-toolbar">
				<!-- 提交号码表单 -->
		...
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="updateNumberForm" action="${pageContext.request.contextPath }/number_updateNumber" method="post">
				<table class="table-edit" width="80%" align="center">
					<tr class="title">
						<td colspan="2">号码信息</td>
					</tr>
	...
	<!-- 查询 -->
	<div class="easyui-window" title="查询号码窗口" id="searchWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="searchform" >
				<table class="table-edit" width="80%" align="center">
					<tr class="title">
					...
					<tr>
						<td>组别</td>
						<td><input type="text" name="grouptype" class="easyui-validatebox"/></td>
					</tr>
					<tr>
